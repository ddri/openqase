...
*   A `div` with `grid grid-cols-1 gap-12 lg:grid-cols-12` creates a two-column layout on large screens.
    *   **Left Column (Main Content - `lg:col-span-9`):**
        ...
        *   **Use Cases Badges:** `div` with `flex flex-wrap gap-2 mb-8`. Shadcn UI `<Badge variant="outline" ...>`
        *   **Main Content (Rendered Markdown):** `div` with `dangerouslySetInnerHTML={{ __html: processedContent }}`. Styled with injected classes and `prose-...` utilities.
        *   **Implementation Steps (`algorithm.steps`):** `<h2>` title, `<StepsRenderer />`.
        *   **Academic References (`algorithm.academic_references`):** `<ReferencesRenderer />`.
    *   **Right Column (Sidebar - `lg:col-span-3`):**
        *   A `div` with `sticky top-8 space-y-8`.
        *   **Related Industries Section:** Displays badges linking to related industry pages (data from `algorithm_industry_relations`).
        *   **Related Personas Section:** Displays badges linking to related persona pages (data from `persona_algorithm_relations`).
        *   **Related Case Studies Section:** `<h3>` title, custom styled linked list items.
...
**C. Admin Algorithm Edit Page (`src/app/admin/algorithms/[id]/...`):**
*   **Functionality:** Allows creation and editing of algorithms, including their textual content, metadata, and relationships.
*   **Key Files:**
    *   `page.tsx`: Server component for fetching initial algorithm data, all available case studies, industries, and personas for relationship selectors.
    *   `client.tsx`: Client component (`AlgorithmForm`) rendering the form fields, including `Input`, `Textarea`, and `RelationshipSelector` components for managing associations.
    *   `actions.ts`: Server actions (`saveAlgorithm`, `publishAlgorithm`, `unpublishAlgorithm`) for handling data persistence and state changes.
*   **Relationship Management:**
    *   **Case Studies:** Uses a `RelationshipSelector` to link algorithms to case studies. Data is stored in `algorithm_case_study_relations`.
    *   **Industries:** Uses a `RelationshipSelector` to link algorithms to industries. Data is stored in `algorithm_industry_relations`.
    *   **Personas (Newly Added):** A `RelationshipSelector` was added to `client.tsx` for linking algorithms to personas. The `page.tsx` fetches all available personas and the current algorithm's related persona IDs to populate this selector. The `saveAlgorithm` function in `actions.ts` was updated to handle saving these relationships to the `persona_algorithm_relations` junction table, following the established delete-then-insert pattern.
...
*   *[Current Date]*: Documented admin functionality for linking Personas to Algorithms and display on public Algorithm page.
...
*   **Linking Convention (Detail Page Sidebars):** Classification badges (e.g., Related Industries, Related Algorithms, Related Personas) displayed in the sidebars of individual detail pages (Case Studies, Algorithms, Personas) link to their respective listing or detail pages (e.g., a "Finance" industry badge on a Case Study page links to `/paths/industry/finance`, a "Domain Expert" persona badge on an Algorithm page links to `/paths/persona/domain-expert`). This pattern is consistent across these content types.

**10. Horizontal Rules (Section Separators):**
*   **Purpose:** Used to create a clear visual break between distinct major content blocks on a page, particularly between primary descriptive/narrative content and subsequent supplementary sections (e.g., "Recommended Reading," "Related Case Studies" on Persona pages).
