...
const CONTENT_TYPE = 'quantum_hardware' as const;

// Define relationship configurations for this content type
const RELATIONSHIP_CONFIG = {
  caseStudies: RELATIONSHIP_CONFIGS.quantumHardware?.caseStudies
};

/**
 * GET handler for fetching quantum hardware
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (slug) {
      ...
      if (error || !data) {
        return NextResponse.json(
          { error: 'Quantum hardware not found' },
          { status: 404 }
        );
      }
    ...
    if (error) {
      return NextResponse.json(
        { error: 'Error fetching quantum hardware' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Failed to fetch quantum hardware' },
      { status: 500 }
    );
  }
}
...
/**
 * POST handler for creating quantum hardware
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Save the quantum hardware item
    ...
    if (error || !savedItem) {
      console.error('Error creating quantum hardware:', error);
      return NextResponse.json(
        { error: 'Failed to create quantum hardware' },
        { status: 500 }
      );
    }
    ...
    console.error('Error in quantum-hardware POST handler:', error);
    return NextResponse.json(
      { error: 'Failed to create quantum hardware' },
      { status: 500 }
    );
...
/**
 * PUT handler for updating quantum hardware
 */
export async function PUT(request: NextRequest) {
  try {
    ...
    // Update the quantum hardware item
    const { data: savedItem, error } = await saveContentItem({
      contentType: CONTENT_TYPE,
      data: itemData,
      id,
    ...
    if (error || !savedItem) {
      console.error('Error updating quantum hardware:', error);
      return NextResponse.json(
        { error: 'Failed to update quantum hardware' },
        { status: 500 }
      );
    }
    ...
    console.error('Error in quantum-hardware PUT handler:', error);
    return NextResponse.json(
      { error: 'Failed to update quantum hardware' },
      { status: 500 }
    );
